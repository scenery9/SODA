...
<p align="center">
  <img src="assets/figure-5-3-three-week-build-plan.png" alt="Figure 5.3: A three-week build plan, drawn as four owner rows across three week columns, totalling 120 team hours. Week 1, Establish the core. Boon Shen: Supabase schema and Row Level Security, token validation on every request, the five-axis load model. Jia Yin: Flutter shell and navigation, task entry with effort and priority, My Backpack. Samantha: onboarding calibration, Recovery Island layout, synthetic demo data. Ikhlas: test plan and fixtures, two-account isolation check, arithmetic and threshold edges. Gate 1: sign in, save a manual task, reproduce the load view, and two accounts cannot read each other's rows. Week 2, Complete the decision loop. Boon Shen: Impact Preview simulation, constrained Smart Rebalance, atomic apply, undo and the recovery ledger. Jia Yin: Impact Preview screen, Smart Rebalance and Protection Mode, Life Forecast and Day Detail. Samantha: recovery choice and protected time, completion logging, Recovery Debt wired to the backend. Ikhlas: scheduling constraint checks, stale apply and undo regression, no-feasible-plan handling. Gate 2: preview, approve, persist and undo safely, and a recovery session logs once and updates the ledger. Week 3, Integrate and rehearse. Boon Shen: defect fixes, fixture verification, FastAPI deploy on Railway. Jia Yin: accessible interaction states, text alternatives on every chart, consistency pass against Figma. Samantha: Flutter web deploy, README, slides and demonstration, final submission assembly. Ikhlas: keyboard and TalkBack review, offline and language-disabled rehearsal, defect tracking and retest. Gate 3: core defects closed, accessibility pass, rehearsed with AI switched off on another network." width="980">
</p>
