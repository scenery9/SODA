...
| Recovery stays an unprotected intention | Specifying when and where an action happens supports follow-through (Gollwitzer, 1999) | Recovery Island turns the intention into a chosen action with a protected interval, which Smart Rebalance then refuses to move |
...
**Limits of the claim.** The model depends on recorded information, so missing commitments produce false
...
Student-burnout and workload research that shaped our understanding of the problem but is not cited as the basis of any specific design decision above.
